...
vec pv1[N], pv2[N], cv1[N], cv2[N], cv[N], qv[N];
...
int main(void) {
    ...
    int m = minkowski_sum(cv1, m1, cv2, m2, cv);


    cvq::build(cv, m);

    for (int i = 1; i <= q; ++i) {
        cout << cvq::contain(qv[i]) << endl;
    }

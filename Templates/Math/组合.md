...
定理(Caylay)：$n$个点的无标号有根树共有$n^{n-2}$种。从Prufer序列的构造与还原不难看出一棵树对应着一个Prufer序列，而每个Prufer序列在其$n-2$个位置均有$n$种取值可能。
...
设$F(n,k,S)$为缩点后有且仅有$S$中点组成了$k$出度为$0$的有向图数量，$G(n,k,S)$为缩点后至少有$S$中点出度为0，且$S$中点不与其他点组成强连通分量的的有向图数量，则

$$G(n,S)=2^{|S|(n-|S|)}$$

$$G(n,S)=\sum_{T\supseteq S}{F(T)}$$
...
设$F(n,i)$表示$n$个点组成$i$个互不相连的强连通分量的方案数，则有

$$F(n,1)=H(n)-\sum_{k=1}^{n}{\binom n k 2^{k(n-k)}H(n-k)\sum_{i=1}^{k}{F(k,i)}}$$

$$F(n,k)=\sum_{i=1}^{n-k+1}F(n-i,k-1)F(n-i,1)$$

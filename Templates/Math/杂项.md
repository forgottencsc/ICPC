...
int inv(ll x) { return x == 1 ? 1 : mul(inv(P % x), P - P / x); }
int qpm(int a, ll b) {
    int r = 1;
    do if (b & 1) r = mul(r, a); while (a = mul(a, a), b >>= 1);
    return r;
}

int invs[N], f[N], fi[N], bi[N][N], be[N], ep[N-1][N];
void init() {
    invs[1] = f[0] = fi[0] = 1;
    for (int i = 2; i != N; ++i) invs[i] = mul(invs[P % i], P - P / i);
    for (int i = 1; i != N; ++i) f[i] = mul(i, f[i - 1]);
    for (int i = 1; i != N; ++i) fi[i] = mul(invs[i], fi[i - 1]);
    // Binomial coefficient
    for (int i = 0; i != N; ++i)
        for (int j = bi[i][0] = bi[i][i] = 1; j < i; ++j)
            bi[i][j] = add(bi[i - 1][j - 1], bi[i - 1][j]);
    //  Bernoulli numbers
    for (int i = be[0] = 1; i != N; ++i)
        for (int j = 0; j != i; ++j)
            be[i] = sub(be[i], mul(bi[i][j], mul(be[j], invs[i - j + 1])));
    //  Equal Power Sum Coef
    for (int i = ep[0][0] = ep[0][1] = 1; i != N - 1; ++i)
        for (int j = 0; j <= i; ++j)
            ep[i][i+1-j]=(j&1?sub:add)(0, mul(mul(bi[i + 1][j], be[j]), invs[i+1]));
}
```
...
```cpp
int epsum(int n, int k) {
    int w = 1, r = 0;
    for (int i = 0; i <= k + 1; ++i, w = mul(w, n))
        r = add(r, mul(w, ep[k][i]));
    return r;
}
```
...
给出一个字符集$\Sigma$和由该集生成的字符串和一些变换法则，如：

$\Sigma=\{'0','1'\}$

1. 在任意位置插入或删除"00"
2. 在任意位置插入或删除"111"
3. 在任意位置插入或删除"0101010101"

给定两个串，判定第一个能否通过这些变换法则变成第二个串。

不难发现所有串形成了一些等价类，且这些等价类之间存在群结构。这个例子中的群似乎是$A_5$。

于是我们要做的就是通过暴力找到这个群的置换表示，即找出生成元$'0'$与$'1'$对应的置换，则每个串可被视作一串$x$或$y$的积。
...
int det(mat& a) {
    ...
    for (int i = 0; i != n; ++i) {
        for (int j = i + 1; !a[i][i] && j != n; ++j)
            if (a[j][i]) swap(a[i], a[j]), r = sub(0, r);
        if (!a[i][i]) return 0; int w = inv(a[i][i]);
        for (int j = i + 1; j != n; ++j) {
            int t = mul(a[j][i], w);
            for (int k = i; k != n; ++k)
...
int det(mat& a) {
    ...
    for (int i = 0; i != n; ++i) {
        ...
        for (int j = i + 1; j != n; ++j) {
            while (a[j][i]) {
                swap(a[i], a[j]); r = sub(0, r);
                int t = a[j][i] / a[i][i];
                if (t) for (int k = i; k != n; ++k)
                    a[j][k] = sub(a[j][k], mul(a[i][k], t));
            }
        }
        r = mul(r, a[i][i]);
    }
...
注：`eigenvalues`只能用来求实对称阵的特征值，迭代次数可以根据调整。
...
pair<mat, mat> QR(const mat& a) {
    mat q = transpose(a); gso(q);
    return { transpose(q), q * a };
}

vec eigenvalues(mat a) {
    for (int i = 0; i != 20; ++i) {
        mat q = transpose(a); gso(q);
        a = q * a * transpose(q);
    }
...
$$T \subseteq U, |U| = n, |T| = r$$

```cpp
for (int t= (1 << r) - 1; t < (1 << n);) {
    //  ...
    int y= t + (t & -t);
    t=((t & ~y) >> (__builtin_ctz(t) + 1)) | y;
}
```
...
对性能要求高需要在前面吟唱NTT，否则请手动暴力卷积或抄上$O(n^{1.57})$的Karatsuba。

使用NTT进行卷积时除法与乘法的复杂度均为$O(n \log n)$，其中$n$为位数。

除法常数大小请自行脑补，$n=10^5$时在51nod上要跑0.6s。

`rad`为进制，改成100或1000并不能加速反而会挂，我也不知道为什么。

...
给定$f(x)=\sum_{i=0}^n{a_ix^i}$，求$f(x+c)$。

$$
f(x+c)=\sum_{i=0}^n{a_i\sum_{j=0}^i{\binom ij x^j c^{i-j}}}
=\sum_{j=0}^n x^j \sum_{i=j}^n{\binom ij a_i c^{i-j}}
$$

$$
=\sum_{j=0}^n x^j \sum_{i=0}^{n-j} \binom {n-i}{j}a_{n-i}c^{n-i-j}
=\sum_{j=0}^n x^{n-j} \sum_{i=0}^{j} \binom {n-i}{n-j} a_{n-i}c^{j-i}
$$

令$b_i=a_{n-i}$

$$
=\sum_{j=0}^n \frac{x^{n-j}}{(n-j)!}\sum_{i=0}^j \frac{(n-i)!}{(j-i)!}b_i c^{j-i}
=\sum_{j=0}^n\frac{x^{n-j}}{(n-j)!}\sum_{i=0}^j \left( b_i (n-i)! \frac{c^{j-i}}{(j-i)!}\right)
$$

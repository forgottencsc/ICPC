...
不难发现所有串形成了一些等价类，且这些等价类之间存在群结构。

令$x='0',y='1'$，则每个串可被视作一串$x$或$y$的积。

上述例子所描述的群为$<x^2=e,y^3=e,(xy)^5=e>$，暴力找出分别对应$x$和$y$的排列即可。
...
for (int t=(1<<r)-1; t<(1<<n);) {
    ...
    int y=t+(t&-t); t=((t&~y)>>(__builtin_ctz(t)+1))|y;
}
```
...
对性能要求高需要在前面吟唱NTT，否则请手动暴力卷积。

使用NTT进行卷积时除法与乘法的复杂度均为$O(n \log n)$，其中$n$为位数。

除法常数大小请自行脑补，1e5在51nod上要跑两秒。
...
struct biu : vector<int> {
    ...
    friend biu operator/(const biu& b1, const biu& b2) {
        int s = cmp(b1, b2); if (s != 1) return s + 1;
        int n1 = b1.size(), n2 = b2.size(), n3 = n1;
        biu x; x.assign(n3 + 1, 0); x.back() = 2;
        biu x1; x1.assign(n3 - n2 + 1, 0);
        x1.back() = n2 > 1 ? 100 / (b2.back() * 10 + b2.end()[-2]) : 10 / b2.back();
        while (1) {
            biu x2 = x1 * (x - x1 * b2);
            x2.erase(x2.begin(), x2.begin() + n3);
            if (x2 == x1) break; x1 = move(x2);
        }
        biu b = b1 * x1; b.erase(b.begin(), b.begin() + n3);
        biu r = b2 * b; if (cmp(r + b2, b1) <= 0) b = b + 1; return b;
    }
};
```

...
struct edge { int v, i; dbl w; };

//  Persistent Leftist Tree
int ls[M], rs[M], dep[M], nc; pdi val[M];
int gn(pdi v, int q = 0) {
    int p = ++nc;
    ls[p] = ls[q]; rs[p] = rs[q];
    dep[p] = q ? dep[q] : 1;
    val[p] = q ? val[q] : v;
...
dbl dis[N]; int pre[N], rt[N];

dbl solve(int n, int m, int s, int t, int k,
          const vector<pair<pair<int, int>, double>>& es) {
    for (int i = 1; i <= n; ++i) {
        g[i].resize(0);
        gr[i].resize(0);
        gt[i].resize(0);
    ...
    for (int i = 0; i != es.size(); ++i) {
        pair<pair<int, int>, double> p = es[i];
    ...
    fill_n(dis + 1, n, inf); pq.push({ dis[t] = 0, t });
    fill_n(pre, n, -1);
    while (!pq.empty()) {
        pdi p = pq.top(); pq.pop();
        int u = p.second; dbl du = p.first;
    ...
    for (int u = 1; u <= n; ++u) {
        ...
        for (edge e : g[u]) {
            if (e.i == pre[u]) gt[e.v].push_back(u);
            else rt[u] = merge(rt[u], gn({ dis[e.v] + e.w - dis[u], e.v }));
        }
    }
    ...
    while (!q.empty()) {
        int u = q.front(); q.pop();
        for (int v : gt[u]) rt[v] = merge(rt[v], rt[u]), q.push(v);
    }
    ...
    while (!pq.empty() && --k) {
        pdi p = pq.top(); ans = p.first; pq.pop();
        int x = p.second, u = val[x].second;
        if (rt[u]) pq.push({ ans + val[rt[u]].first, rt[u] });
        if (ls[x]) pq.push({ ans - val[x].first + val[ls[x]].first, ls[x] });
        if (rs[x]) pq.push({ ans - val[x].first + val[rs[x]].first, rs[x] });
    }
    return ans;
}

...
所有树边和前向边都从低DFS序点指向高DFS序点。

所有后向边和横叉边都从高DFS序点指向低DFS序点。
...
等价定义：子图中任意一条边都在至少一个环上。

性质：将任意有向图中所有强连通分量分别缩成点可得到一个DAG。

考虑每个强连通分量在DFS树上的形态：

每个强连通分量都有一个DFS序最低的点。

性质2：该点能仅通过树边到达强连通分量上的其他所有点。

证明：若某点不能仅通过树边到达，则路上必定经过某条从高DFS序指向低DFS序的边$(u,v)$，因为前向边可用树边代替，而其他边。由假设，$v$也不能仅通过树边到达。

强连通分量中每个点均无法到达DFS序更低的点，因为DFS序更低的点只能是该点的

考虑缩点前的图中任意一棵DFS树在缩点后图中的形态，不难发现将缩点后的图展开后每个强连通分量在DFS树上仍然是一个连通块。
